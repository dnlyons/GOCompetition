...
if __name__ == "__main__":
    ...
    for line in raw_data.raw.nontransformer_branches.values():
        ...
        # -- BASE NETWORK -------------------------------------------------------------------------
        idx = pp.create_line_from_parameters(net_a, frombus, tobus, length, r, x, capacitance, i_rating_a, name=linekey, max_loading_percent=99.9, in_service=status)
        # -- CONTINGENCY NETWORK ------------------------------------------------------------------
    ...
    for xfmr in raw_data.raw.transformers.values():
        ...
        idx = pp.create_transformer_from_parameters(net_a, highbus, lowbus, xfmr.rata1, highkv, lowkv, r_pct_a, z_pct_a, pfe_kw=pkw_a, i0_percent=i0_a,
                                                    ...
                                                    in_service=status, name=xfmrkey, max_loading_percent=99.9, parallel=1, df=1.0)
    ...
    # ---------------------------------------------------------------------------------------------
    # -- CREATE INITIAL BASECASE SCOPF ------------------------------------------------------------
    # ---------------------------------------------------------------------------------------------
    print('-------------------- CREATING BASECASE SCOPF 1 ---------------------')
    ...
    nosolve_outages = []                                                                            # INITIALIZE NOSOLVE OUTAGES LIST
    ...
    for gkey in goutagekeys:                                                                        # LOOP THROUGH GENERATOR OUTAGES
        ...
        try:                                                                                        # TRY STRAIGHT POWERFLOW SOLUTION
            ...
            nosolve_outages.append(gkey)                                                                 # APPEND NOT SOLVED OUTAGES LIST
            print(ns_text, '{0:9s} Not Solved with Initial SCOPF Basecase ..............'.format(gkey))  # PRINT INFO ON OUTAGE
    for bkey in boutagekeys:                                                                             # LOOP THROUGH BRANCH OUTAGES
        ...
        try:                                                                                        # TRY STRAIGHT POWERFLOW SOLUTION
            ...
            nosolve_outages.append(bkey)                                                                 # APPEND NOT SOLVED OUTAGES LIST
            print(ns_text, '{0:9s} Not Solved with Initial SCOPF Basecase ..............'.format(bkey))  # PRINT INFO ON OUTAGE

    # ---------------------------------------------------------------------------------------------
    # -- FIND OUTAGES RESULTING IN LOADING CONSTRAINTS --------------------------------------------  # TODO and voltage constraints
    # ---------------------------------------------------------------------------------------------
    print('------------ CHECK FOR OUTAGES WITH LOADING CONSTRAINTS ------------')
    ...
    for scopf_key in solved_outages:                                                                # LOOP ACROSS SOLVED CONTINGENCIES
        ...
        try:                                                                                        # TRY STRAIGHT POWERFLOW SOLUTION
            ...
            print(scopf_text, '{0:9s} NOT SOLVED USING INITIAL SCOPF BASECASE .............'.format(scopf_key))     # PRINT INFO ON OUTAGE
    ...
    while constraint and step < 2:                                                                  # LOOP WHILE CONSTRAINT EXISTS
        ...
        for outage_key in constrained_outages:                                                      # LOOP THROUGH CONSTRAINED OUTAGES
            constrained_outages1 = []                                                               # INITIALIZE A 'SUB' CONTRAINED OUTAGES LIST
            net = copy.deepcopy(scopf_net)                                                          # GET FRESH COPY OF SCOPF NETWORK
            if outage_key in online_gens:                                                           # CHECK IF A GENERATOR...
                scopf_text = 'GEN '                                                                 # ASSIGN TEXT
                gidx = gen_dict[outage_key]                                                         # GET GENERATOR INDEX
                net.gen.in_service[gidx] = False                                                    # SWITCH OFF OUTAGED GENERATOR
            elif outage_key in line_dict:                                                           # CHECK IF A LINE...
                scopf_text = 'LINE'                                                                 # ASSIGN TEXT
                lineidx = line_dict[outage_key]                                                     # GET LINE INDEX
                net.line.in_service[lineidx] = False                                                # SWITCH OUT OUTAGED LINE
            elif outage_key in xfmr_dict:                                                           # CHECK IF A XFMR...
                scopf_text = 'XFMR'                                                                 # ASSIGN TEXT
                xfmridx = xfmr_dict[outage_key]                                                     # GET XFMR INDEX
                net.trafo.in_service[xfmridx] = False                                               # SWITCH OUT OUTAGED XFMR
            ...
            max_loading = max(line_loading, xfmr_loading)                                           # GET MAX OF MAXs
            if max_loading > 100.0:                                                                                                  # IF MAX LOADING < 100%
                print(scopf_text, '{0:9s} Outage Results in Loading Constraint ({1:.1f}%) .......'.format(outage_key, max_loading))  # PRINT INFO ON OUTAGE
                constraint = True                                                                                                    # SET CONSTRAINT FLAG
                constrained_outages1.append(outage_key)                                             # ADD OUTAGE TO 'SUB CONSTRAINED OUTAGES LIST
                try:                                                                                # TRY TO SOLVE WITH OPF
                    pp.runopp(net, init='pf', enforce_q_lims=True)                                  # RUN OPF ON THIS NETWORK
                except:                                                                                                     # IF NO SOLUTION...
                    print(scopf_text, '{0:9s} DID NOT SOLVE WITH OPF ..............................'.format(outage_key))    # PRINT STATEMENT
                    continue                                                                                                # GET NEXT CONTRAINED OUTAGE
    ...
    # -- FOR BRANCH OUTAGES ---------------------------------------------------                     # TODO ? ----------------------------------------
    # -- ATTEMPT TO INSURE AREA GENERATORS HAVE ADEQUATE UPMARGIN -------------
    # adj_units = []
    # for bkey in boutagekeys:
    #     participating_units = []                                                                    # INITIALIZE PARTICIPATING GENERATOR LIST
    #     outage_areas = branch_areas[bkey]                                                           # GET AREAS CONNECTED TO BRANCH
    ...
    #         if gkey not in adj_units:                                                               # IF GENERATOR HAS NOT ALREADY BEEN ADJUSTED...
    #             gidx = gen_dict[gkey]                                                               # GET PARTICIPATING GENERATOR INDEX
    #             pmax = net.gen.loc[gidx, 'max_p_mw']                                                # GET THIS GENERATOR PMAX
    #             net.gen.loc[gidx, 'max_p_mw'] = pmax - 2.5                                          # REDUCE PMAX BY SOME AMOUNT
    #             net_a.gen.loc[gidx, 'max_p_mw'] = pmax - 2.5                                        # REDUCE PMAX BY SOME AMOUNT
    #             net_c.gen.loc[gidx, 'max_p_mw'] = pmax - 2.5                                        # REDUCE PMAX BY SOME AMOUNT
    #             adj_units.append(gkey)                                                              # ADD THIS GENERATOR TO ADJUSTED GENERATORS
    # pp.runpp(net, enforce_q_lims=True)                                                              # THIS NETWORK, RUN STRAIGHT POWER FLOW

    # -- FOR GENERATOR OUTAGES -----------------------------------------------                      # TODO ? ----------------------------------------
    # -- ATTEMPT TO INSURE GENERATORS HAVE ADEQUATE UPMARGIN -----------------
    # for a in area_participating_gens:                                                             # LOOP ACROSS PARTICIPATING GENERATORS
    #     area_gkeys = area_participating_gens[a]
    #     for gkey in gkeys
    #     gidx = gen_dict[gkey]                                                                     # GET PARTICIPATING GENERATOR INDEX
    #     pmax = net.gen.loc[gidx, 'max_p_mw']                                                      # GET THIS GENERATOR PMAX

    # -- RUN SCOPF AGAIN WITH ADJUSTMEMTS -------------------------------------
    print('-------------------- CREATING BASECASE SCOPF 2 ---------------------')
    net = copy.deepcopy(net_a)
    pp.runopp(net, init='pf', enforce_q_lims=True)                                                  # RUN OPF ON THIS NETWORK
    net.gen['p_mw'] = net.res_gen['p_mw']                                                           # SET THIS NETWORK GENERATORS POWER TO OPF RESULTS
    net_a.gen['p_mw'] = net.res_gen['p_mw']                                                         # SET THIS NETWORK GENERATORS POWER TO OPF RESULTS
    ...
    # -- RUN SCOPF AGAIN WITH ADJUSTMEMTS -------------------------------------
    print('------------------ CREATING FINAL BASECASE SCOPF -------------------')
    pp.runopp(net, init='pf', enforce_q_lims=True)                                                  # RUN OPF ON THIS NETWORK (AGAIN)
    net.gen['p_mw'] = net.res_gen['p_mw']                                                           # SET THIS NETWORK GENERATORS POWER TO OPF RESULTS
    net_a.gen['p_mw'] = net.res_gen['p_mw']                                                         # SET THIS NETWORK GENERATORS POWER TO OPF RESULTS
    net_c.gen['p_mw'] = net.res_gen['p_mw']                                                         # SET THIS NETWORK GENERATORS POWER TO OPF RESULTS
    for gkey in gen_dict:                                                                           # LOOP ACROSS GENERATOR KEYS
    ...
    init_cost = get_generation_cost(net_a, participating_gens, gen_dict, pwlcost_dict0)
    ...
    init_max_loading = max(line_loading, xfmr_loading)                                              # GET MAX OF MAXs
    ...
    print('FINAL BASECASE SCOPF CREATED ........... ${0:6.0f}  {1:5.1f}% ...........'.format(init_cost, init_max_loading), round(time.time() - opf_startime, 3))
    print('SWING VREG SETPOINT = {0:.4f} .......................................'.format(max_v))
    ...
    zeroed = not ((abs(external_pgen) < external_pgen_threshold) or (abs(external_qgen) < external_qgen_threshold))     # CHECK IF EXTGRID IS ZEROED
    step = 1                                                                                        # INITIALIZE ITERATOR
    while not zeroed and step < 120:                                                                # LIMIT WHILE LOOP ITERATIONS
        zeroed = True                                                                               # SET ZEROED FLAG
    ...
    pickle.dump([base_pgens, ext_grid_idx, genbuses, genbus_dict, Gids, line_dict, goutagekeys, boutagekeys,
                 outage_dict, gen_dict, pfactor_dict, xfmr_dict, area_swhunts, swsh_dict, swshbus_dict, swshidxs,
                 swshidx_dict, busarea_dict, branch_areas, area_participating_gens, swinggen_idxs, alt_sw_genkey, tie_idx, alt_tie_idx], PFile)      # WRITE MISC DATA TO PICKLE FILE
