...
# -----------------------------------------------------------------------------
if not sys.argv[1:]:
    con_fname = cwd + r'/sandbox/Network_07R-10/scenario_1/case.con'
    inl_fname = cwd + r'/sandbox/Network_07R-10/case.inl'
    raw_fname = cwd + r'/sandbox/Network_07R-10/scenario_1/case.raw'
    rop_fname = cwd + r'/sandbox/Network_07R-10/case.rop'
    outfname1 = cwd + r'/sandbox/Network_07R-10/scenario_1/solution1.txt'
    outfname2 = cwd + r'/sandbox/Network_07R-10/scenario_1/solution2.txt'
    neta_fname = cwd + r'/sandbox/Network_07R-10/scenario_1/neta.p'
    netc_fname = cwd + r'/sandbox/Network_07R-10/scenario_1/netc.p'
    data_fname = cwd + r'/sandbox/Network_07R-10/scenario_1/netdata.pkl'
...
def get_alt_swingbus(generators, buses, swbus, sw_kv, sw_reg):
    ...
    swbus1 = None
    sw1gkey = None
    minmaxq_list = [[g.qb, g.qt, g.i, str(g.i) + '-' + g.id] for g in generators.values() if g.i != swbus]
    minmaxq_list.sort()
    for q in range(len(minmaxq_list)):
        gbus = minmaxq_list[q][2]
        for bus in buses.values():
            if bus.i == gbus:
                minmaxq_list[q].append(bus.baskv)
    sw_candidates = [g for g in minmaxq_list if g[4] == sw_kv]
    sw_candidates.sort()
    for j in sw_candidates:
        if j[0] == -j[1]:
            swbus1 = j[2]
            sw1gkey = j[3]
            break
...
if __name__ == "__main__":
    ...
    # line = (line.i, line.j, line.ckt, line.r, line.x, line.b, line.ratea, line.ratec, line.st, line.len)
    line_dict = {}
    line_ratea_dict = {}
    lineidxs = []
    print('ADD LINES ..........................................................')
    ...
    net_a.gen['p_mw'] = net_a.res_gen['p_mw']                                                       # SET THIS NETWORK GENERATORS POWER TO OPF RESULTS
    net_c.gen['p_mw'] = net_a.res_gen['p_mw']                                                       # SET THIS NETWORK GENERATORS POWER TO OPF RESULTS
    for gkey in gen_dict:                                                                           # LOOP ACROSS GENERATOR BUSES
        gidx = gen_dict[gkey]
        genbus = genbus_dict[gidx]
    ...
    for shkey in swsh_dict:
        shidx = swsh_dict[shkey]
        shbus = swshbus_dict[shidx]
